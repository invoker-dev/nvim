...
local minifiles_toggle = function()
  -- Try to close MiniFiles; if it was open, this returns true
  if not MiniFiles.close() then
    local path = vim.api.nvim_buf_get_name(0)
    local stat = vim.loop.fs_stat(path)

    -- If the current file exists, open MiniFiles in its directory
    if path ~= "" and stat and stat.type == "file" then
      MiniFiles.open(vim.fn.fnamemodify(path, ":h"))
    else
